...
class WidgetConfigActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            val isCard =
                glanceManager.getGlanceIds(ArticleCardWidget::class.java).contains(glanceId)
            val config = repository.getConfig(appWidgetId)
            showConfig(config, glanceId, appWidgetId, isCard)
        }
    }
        ...
        setContent {
            settingsProvider.ProvidesSettings {
                AppTheme(useDarkTheme = LocalDarkTheme.current.isDarkTheme()) {
                    var config: WidgetConfig by remember { mutableStateOf(initialConfig) }
