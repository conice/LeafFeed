...
import com.conice.morss.domain.model.article.ArchivedArticle
...
import com.conice.morss.domain.model.group.GroupWithFeed
...
abstract class AbstractRssRepository(
        ...
        return when {
            ...
            groupId != null ->
                when {
                    ...
                    isUnread ->
                        if (sortAscending) {
                            articleDao.queryArticleWithFeedByGroupIdWhenIsUnreadAscending(
                                accountId,
                                groupId,
                                true,
                            )
            ...
            feedId != null ->
                when {
                    ...
                    isUnread ->
                        if (sortAscending) {
                            articleDao.queryArticleWithFeedByFeedIdWhenIsUnreadAscending(
                                accountId,
                                feedId,
                                true,
                            )
                ...
                when {
                    ...
                    isUnread ->
                        if (sortAscending) {
                            articleDao.queryArticleWithFeedWhenIsUnreadAscending(accountId, true)
                        } else {
                            articleDao.queryArticleWithFeedWhenIsUnread(accountId, true)
                        }
    ...
    fun pullImportant(
        ...
        val accountId = accountService.getCurrentAccountId()
        val audioOnly = contentType == ArticleContentType.AUDIO
        Log.i(
            "RLog",
            "pullImportant: accountId: ${accountId}, isStarred: ${isStarred}, isUnread: ${isUnread}",
        )
        return when {
            isStarred -> articleDao.queryImportantCountWhenIsStarred(accountId, true, audioOnly)
            isUnread -> articleDao.queryImportantCountWhenIsUnread(accountId, true, audioOnly)
            else -> articleDao.queryImportantCountWhenIsAll(accountId, audioOnly)
        }
    }
    ...
    suspend fun isFeedExist(url: String): Boolean =
        feedDao.queryAll(accountService.getCurrentAccountId()).any {
            feedUrlsEquivalent(it.url, url)
        }
