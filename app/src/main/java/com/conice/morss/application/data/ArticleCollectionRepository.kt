...
import com.conice.morss.domain.model.article.ArticleTagGroup
...
import com.conice.morss.domain.model.general.getDefaultGroupId
...
constructor(
    ...
    suspend fun exportBackup(): String {
        ...
        val readingStateRows = articleBackupDao.queryReadingStatesForBackup(accountId)
        val stateIdentities =
        ...
        val otherIdentities =
            referencedArticleIds
                ...
                .flatMap { articleBackupDao.queryBackupIdentities(accountId, it) }
    ...
    suspend fun importBackup(content: String): ArticleCollectionImportResult {
        ...
        val currentByPortableIdentity =
            backupLinks
                .chunked(ARTICLE_QUERY_CHUNK_SIZE)
                .flatMap { articleBackupDao.queryBackupIdentitiesByLinks(accountId, it) }
        ...
        articleBackupDao.restoreReadingStates(readingStates)
...
internal const val COLLECTION_BACKUP_FORMAT = "morss.collections"
internal const val COLLECTION_BACKUP_VERSION = 6
internal const val AUTOMATION_BACKUP_FORMAT = "morss.automations"
...
private const val PORTABLE_ID_NAMESPACE = "Morss"

private fun portableImportedId(kind: String, accountId: Int, sourceId: String): String =
    UUID.nameUUIDFromBytes(
        "$PORTABLE_ID_NAMESPACE:$kind:$accountId:$sourceId".toByteArray(Charsets.UTF_8)
    )
        .toString()
