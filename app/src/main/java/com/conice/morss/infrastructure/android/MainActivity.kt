...
import com.conice.morss.infrastructure.widget.WidgetUpdateWorker
import com.conice.morss.infrastructure.compose.ProvideCompositionLocals
import com.conice.morss.infrastructure.datastore.getOrDefault
import com.conice.morss.infrastructure.preference.AccountSettingsProvider
...
import com.conice.morss.infrastructure.preference.LocalDarkTheme
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Increase the cursor window size for large article/database operations.
        try {
        ...
        lifecycleScope.launch {
            ...
            accountService.awaitCurrentAccount()
            val isFirstLaunch =
                preferences.getOrDefault(PreferencesKey.isFirstLaunch, true)
            val initialPage = InitialPagePreference.fromPreferences(preferences)

            if (Build.VERSION.SDK_INT < 33) {
                LanguagesPreference.fromPreferences(preferences).let {
                    ...
                    settingsProvider.ProvidesSettings {
                        ...
                        ProvideCompositionLocals {
                            AppTheme(useDarkTheme = LocalDarkTheme.current.isDarkTheme()) {
                                ...
                                val backStack =
                                    rememberNavBackStack(*startDestination.toTypedArray())

                                NewIntentHandlerEffect(backStack, subscribeViewModel)
                                AppEntry(backStack, podcastPlayer)
                            }
                        }
                    }
                }
                    ...
                    intent.getLaunchAction()?.let { action ->
                        when (action) {
                            ...
                            is LaunchAction.OpenArticleList -> {
                                ...
                                if (
                                    accountId != null &&
                                        accountId != accountService.currentAccountIdFlow.value
                                )
                                    return@Consumer
                                filterUseCase.init(feedId, groupId)
...
private fun extractSharedUrl(text: String): String? {
    val value = text.trim()
    if (value.isBlank()) return null
    return Patterns.WEB_URL.matcher(value).takeIf { it.find() }?.group()
